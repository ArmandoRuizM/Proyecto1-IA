...
#Función clave de ordenamiento por costo y heuristica
def costH(a):
    return a[1]
...
def AStarSolve():
    ...
    AStarQueue.append([AStarNode(initStatus(maze), None, None, 0, 0), h(AStarNode(initStatus(maze), None, None, 0, 0))])
    ...
    while go:
        ...
        if n.solution(): #Se determina si el nodo raíz es meta
            ...
            if not createdNodes[3] is None:
                if not (isIn(createdNodes[3], AStarQueue)):
                    AStarQueue.append([createdNodes[3],createdNodes[3].getCost()+h(createdNodes[3])])
            if not createdNodes[2] is None:
                if not (isIn(createdNodes[2], AStarQueue)):
                    AStarQueue.append([createdNodes[2],createdNodes[2].getCost()+h(createdNodes[2])])
            if not createdNodes[1] is None:
                if not (isIn(createdNodes[1], AStarQueue)):
                    AStarQueue.append([createdNodes[1],createdNodes[1].getCost()+h(createdNodes[1])])
            if not createdNodes[0] is None:
                if not (isIn(createdNodes[0], AStarQueue)):
                    AStarQueue.append([createdNodes[0],createdNodes[0].getCost()+h(createdNodes[0])])
